...
const saveSensorData = async (dataPayload) => {
  try {
    ...
    // Falls Sensoren 0.0 sind, ignorieren wir sie fŸr die DB-Speicherung nicht (Rohdaten),
    ...
    const newLog = new SensorLog({
      device: "esp32_main",
      readings: {
        // HAUPTWERTE (Wichtig fŸr Diagramme!)
        temp: avgTemp, 
        humidity: avgHum,

        // Einzel-Sensoren (Rohdaten fŸr Details)
...
// API Route: Gibt Sensor-Historie mit Pagination zurŸck
